...
export default {
  ...
  methods: {
    save() {
      ...
      if (this.name === null || this.name && this.name.replace(/(^\s*)|(\s*$)/g, '') === '') {
        this.$message({
          type: 'warning',
          message: '请输入名称！'
        });
      ...
      const editTreeTtem = { ...this.item, isEdit: false, name: this.name.replace(/(^\s*)|(\s*$)/g, '') };
      this.$store.dispatch('productCate/checkRepeatName', editTreeTtem).then((isRepeatName) => {
        if (isRepeatName) {
          this.$message({
            type: 'warning',
            message: '名称有重复！'
